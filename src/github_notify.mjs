...
try {
  ...
  // Notifications are best-effort: a GitHub API failure (Issues disabled, a
  // read-only token, rate limiting, or a transient 5xx) must never fail the
  // monitor job after a successful scan and workbook build.
  console.error(`GitHub notification warning (non-fatal): ${error.message}`);
}
