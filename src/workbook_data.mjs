...
export async function workbookSheets(root) {
  ...
  const auditNewest = newestFirst(audit, ["evaluated_at", "last_verified_at"], ["first_seen_at"]);
  const runsNewest = newestFirst(runs, ["run_at"], []);
  ...
  return [
    ...
    { name: "All Extracted Jobs", title: "Current Unfiltered Extraction Snapshot — Newest First", headers: decisionHeaders, rows: extractedNewest.map(decisionRow), widths: [20,20,13,30,40,26,16,16,16,44,16,14,14,40,20,42,26,14,20,15,15,36,55,44], table: "AllExtractedTable" },
    { name: "Decision Audit", title: `Eligibility Decision Audit — Rolling ${config.decision_history_days || 30} Days — Newest First`, headers: ["Evaluated At", ...decisionHeaders], rows: auditNewest.map(item => [dt(item.evaluated_at), ...decisionRow(item)]), widths: [20,20,20,13,30,40,26,16,16,16,44,16,14,14,40,20,42,26,14,20,15,15,36,55,44], table: "DecisionAuditTable" },
