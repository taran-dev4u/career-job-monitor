...
### TASK-20260825-0030-claude — DONE
- Started: 2026-08-24T22:20:00Z
- Completed: 2026-08-25T00:30:00Z
- Objective: Whole-project defect hunt (three independent parallel audits of `src/lib.mjs`, `src/scrape.mjs`, and `src/monitor.mjs`/`src/notify_ntfy.mjs`/workflow), then repair every confirmed defect, and restructure the system so each of the 17 companies is handled, tuned, validated and alerted on independently.
- Files expected: `src/lib.mjs`, `src/scrape.mjs`, `src/monitor.mjs`, `src/notify_ntfy.mjs`, `companies.json`, `package.json`, `AGENTS.md`
- Files changed: `src/lib.mjs`, `src/scrape.mjs`, `src/monitor.mjs`, `src/notify_ntfy.mjs`, `companies.json`, `package.json`, `tests/notify.test.mjs`, `tests/filter.test.mjs`, `data/state.json`, `index.html`, `outputs/job-monitor/dashboard.html`, `AGENTS.md`
- Files added: `src/company_audit.mjs`, `src/watchdog.mjs`, `tests/companies.test.mjs`, `tests/watchdog.test.mjs`, `tests/company_probe.mjs`
- Files deleted: None.
- Behavior/data impact:
  **P0 — no push notification could fire for any company, at all.** `notificationDecision` was refactored in `514bd03` to accept both a modern 4-arg and a legacy 5-arg call shape, but read `suppress` from the 4th positional slot in both. `src/scrape.mjs:283` uses the modern shape, so `suppress` received the `now` timestamp — a non-empty string, permanently truthy. Every genuinely new eligible job took the suppress branch, was stamped `notification-suppressed eligible baseline` into `state.notified`, and the function returned `false`; `newJobs` was never populated. This is why 19 consecutive runs on 24 Aug reported `new_jobs_added: 0` against 6,089 candidates. Dispatch now keys off the *type* of the second argument. **79 poisoned `state.notified` entries** (uniquely identifiable: that reason string with no `notified_at`, because `now` landed in the wrong parameter) were cleared so those jobs can alert.
  **P0 — US cities sharing a foreign name were rejected as foreign.** The `nonUsCountries` guard's escape hatch only recognised "STATE, United States", not the "City, STATE" form the positive check accepts. Birmingham AL, Manchester NH, Belfast ME, London OH, Dublin OH/CA, Waterloo IA, Vancouver WA, Sydney MT and Paris TX all resolved non-US. Both paths now share `US_COUNTRY_RE` / `US_STATE_ABBR_RE` / `US_STATE_NAME_RE` so they cannot drift apart again.
  **P0 — unrelated uses of "sponsor" zeroed out eligibility.** The visa/immigration/employment context was optional, so "we are unable to sponsor athletic events this year" matched and stamped the job `Sponsorship: Not Available`. Context is now required.
  **P0 — graduate-eligible internships were rejected.** "must be enrolled in a degree program OR a recent graduate within 6 months" matched only the enrolled clause. A genuine `or` disjunction with a graduation term on either side now qualifies; "enrolled AND graduating in 2027" is still correctly rejected.
  **P0 — dual-level titles were excluded wholesale.** "Entry Level & Senior Software Engineer, AI Software Platform" (a real Qualcomm req, tagged Entry/Mid by Qualcomm) and "Software Engineer (New Grad / Experienced Levels)" were rejected on the seniority word. An explicit entry-level qualifier in the title now overrides the exclusion.
  **P1 — company history read as a candidate requirement.** "Founded in 1998, our company has over 25 years of experience" parsed as *requires 25 years*. Added `COMPANY_HISTORY_RE`. Also, a range "2-8 years" took the lower bound and wrongly accepted; it now uses the upper bound.
  **P1 — invalid and future dates.** `new Date("02/30/2026")` silently became 2 March and a typo'd "12/31/2099" became a brand-new posting. `parseJobDate` now routes every absolute-date path through one validated helper that rejects rolled-over and far-future dates, and the permissive fallback can no longer resurrect a rejected string.
  **P1 — jobs permanently lost by the notifier, two ways.** (a) `monitor.mjs` marks a job notified on first sight whether or not a push went out, so a job carrying an old posting date the *first* time it was seen was suppressed and never offered again; a never-pushed job now gets exactly one catch-up alert regardless of age. (b) `MAX_PER_RUN` capped each run at 20 with a summary naming only 15 more, so on a real 48-job run 13 jobs arrived as a bare number and were never re-offered; overflow now persists to `data/pending_push.json` and drains from the front of the queue on subsequent runs.
  **P1 — browser context leak.** `browser.newContext()`/`newPage()` ran before the try block; a failure in `newPage()` leaked the context into the single browser shared by all 17 companies, and enough leaks would fail every subsequent company. Creation is now leak-safe.
  **P1 — high-volume sources could starve their own backlog.** Detail evaluation ran in raw DOM order, so a source posting more than `max_new_details` jobs per cycle pushed its own unevaluated backlog further down the "most recent" listing every run. The queue is now ordered never-evaluated-first, oldest-discovery-first.
  **P1 — unbounded state growth.** `discovered`/`evaluated`/`notified` were append-only; `state.json` is 4.3 MB and rewritten in full every ~30 min (`.git` is already 239 MB). Added `pruneState()`, wired into `monitor.mjs`, protecting everything still listed this run or still Active in the catalog. `notified` uses a much longer window than the others, since forgetting a notification causes a duplicate alert.
  **P1 — `writeJsonAtomic` used a shared `${file}.tmp`**, so two concurrent writers could rename a truncated document into place. Now uses a pid+random temp name and cleans up on failure.
  **P0 (per-company independence) — every company is now self-contained.** `companies.json` gained `limits` (max_cards, max_pages, max_new_details, navigation_timeout_ms, settle_time_ms), `expects` (min_candidates, location, date) and a `notes` field recording *why* each differs from the global default; all 17 are tuned individually (Amazon 80 cards/30 details for volume, Workday/Oracle/Phenom 8s settle + 60s nav for slow hydration, Compunnel 30 cards). `companySettings()` resolves per company with global fallback, so an entry without overrides is unchanged and no company's settings can affect another's.
  **New `src/company_audit.mjs`** judges each source against its own declared contract and catches the class of failure that was invisible before: dead location extraction, a page-level date stamped on every job, a collapsed candidate count, a 24h zero-eligible streak, and a broken scan. Run against live data it immediately flagged Amazon's blank locations and shared date — and found a previously unknown bug: **Compunnel's posting date is the string "Upload Resume"**, a button label scraped as a date, on all 10 candidates. Aggregate health reported "17/17 Healthy" throughout. Critical findings now ride in `last_batch.json.audit_alerts` and are pushed to the phone.
